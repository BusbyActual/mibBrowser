...
let mibLoader = (mibs) => {
  mibs.length ? 1==1 : mibs = config[ENV].mibOrder;
  /*
    Load default mibs
  */
...
let buildTree = (dict, start) => {
  ...
  let markParents = (oid) => {
    if(dict[oid]) {
      dict[oid].expanded = true;
      let temp = oid.split('.');
      markParents(temp.slice(0, temp.length - 1).join('.'));
    }
  }
