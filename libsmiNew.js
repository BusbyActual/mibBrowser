...
function smiValue2String(smiValue)
{
  ...
  switch(smiValue.basetype) {
    ...
    case 3 : //SMI_BASETYPE_OBJECTIDENTIFIER: //       = 3,
      vValue = new Uint32Array(smiValue.value.oid.reinterpret( smiValue.len * ref.sizeof.uint).buffer).join('.');
      break;
...
SMILib.smiSetPath('C:/Data/Projects/libsmi/libsmi-0.4.8/mibs/ietf;C:/Data/Projects/libsmi/libsmi-0.4.8/mibs/iana;C:/Data/Projects/libsmi/libsmi-0.4.8/mibs/irtf;C:/Data/Projects/libsmi/libsmi-0.4.8/mibs/site;C:/Data/Projects/libsmi/libsmi-0.4.8/mibs/tubs');
...
var nodeBuff = SMILib.smiGetNode(ref.NULL, '1.3.6.1.2.1.2.2.1.2');
var smiNode = nodeBuff.deref();
...
var typeBuff = SMILib.smiGetNodeType(nodeBuff);
if (typeBuff.length > 0) {
  var smiNodeType = typeBuff.deref();
  console.log('   type->BaseType-> ' + SmiBasetype.get(smiNodeType.basetype).key);
  console.log('   type->Format-> ' + smiNodeType.format);
  var rangeBuff = SMILib.smiGetFirstRange(typeBuff);
  while (rangeBuff.length > 0) {
    var smiRange = rangeBuff.deref();
    console.log('   type->Range-> ' + smiValue2String(smiRange.minValue) + ' to ' + smiValue2String(smiRange.maxValue));
    rangeBuff = SMILib.smiGetNextRange(rangeBuff);
  }
  
  var enumBuff = SMILib.smiGetFirstNamedNumber(typeBuff);
  while (enumBuff.length > 0) {
    var smiEnum = enumBuff.deref();
    console.log('   type->Enum-> ' + smiEnum.name + '(' + smiValue2String(smiEnum.value) + ')');
    enumBuff = SMILib.smiGetNextNamedNumber(enumBuff);
  }
}

var elementBuff = SMILib.smiGetFirstElement(nodeBuff);
while (elementBuff.length > 0) {
  var elementNodeBuff = SMILib.smiGetElementNode(elementBuff);
  if (elementNodeBuff.length > 0) {
    smiElementNode = elementNodeBuff.deref();
    console.log('   element-> ' + smiElementNode.name + '(' + new Uint32Array(smiElementNode.oid.reinterpret(smiElementNode.oidlen * 4).buffer).join('.') + ')');
  }
  elementBuff = SMILib.smiGetNextElement(elementBuff);
}

var refinementBuff = SMILib.smiGetFirstRefinement(nodeBuff);
while (refinementBuff.length > 0) {
  var smiRefinement = refinementBuff.deref();
  console.log('   refinement-> ' + smiRefinement.description);
  refinementBuff = SMILib.smiGetNextRefinement(refinementBuff);
}

var relatedNodeBuff = SMILib.smiGetRelatedNode(nodeBuff);
if (relatedNodeBuff.length > 0) {
  var smiReleatedNode = relatedNodeBuff.deref();
  console.log('   related node-> ' + smiReleatedNode.name + '(' + new Uint32Array(smiReleatedNode.oid.reinterpret(smiReleatedNode.oidlen * 4).buffer).join('.') + ')');
